...
def parse_month_start(s: str) -> Optional[pd.Timestamp]:
    """Parse first date from a range string. Returns None if unparseable."""
    try:
        cleaned = str(s).split(" - ")[0].strip()
...
def make_month_label(s) -> Optional[str]:
    """
    Convert a date/range string to 'March 2026'.
    Returns None for invalid/empty values (Total rows etc.).
    """
    if pd.isna(s) if not isinstance(s, str) else (not s.strip()):
        return None
    dt = parse_month_start(str(s))
    if dt is None:
        return None
    try:
...
def run_overall_view(
    ...
    # ── Month labels — apply safely, drop rows where label is None ──
    if meta_month_col:
        meta["_month"] = meta[meta_month_col].apply(make_month_label)
        meta = meta[meta["_month"].notna()].copy()  # drop Total/empty rows

    if shop_month_col:
        shopify["_month"] = shopify[shop_month_col].apply(make_month_label)
        shopify = shopify[shopify["_month"].notna()].copy()

    if has_google and goog_month_col:
        google["_month"] = google[goog_month_col].apply(make_month_label)
        google = google[google["_month"].notna()].copy()

    has_month = bool(
        meta_month_col and shop_month_col and
        (not has_google or goog_month_col) and
        "_month" in meta.columns and "_month" in shopify.columns and
        len(meta) > 0 and len(shopify) > 0
    )
    ...
    if has_month:
        ...
        if has_google:
            google_g = (
                ...
                .agg({"_cost":"sum","_conv":"sum","Google Item ID":"first"})
                .reset_index()
            )
            merged = (
                meta_g.merge(shopify_g, on=["_pid","_month"], how="outer")
        ...
        if has_google:
            ...
            merged = (
                meta_g.merge(shopify_g, on="_pid", how="outer")
    ...
    if has_google and "Google Item ID" in google.columns:
        ...
        merged["Google Item ID"] = (
            merged["Product ID"].apply(clean_pid).map(google_item_map).fillna("")
        )
    ...
    # ── Final column order ───────────────────────────────────────────
    keep = ["Product ID"]
    if has_google and "Google Item ID" in merged.columns:
        keep.append("Google Item ID")
    keep += ["Product Title", "Variant Title"]
    if has_month:
        keep.append("Month")
    keep += [
...
def run_discount_analysis(
    ...
    discount: pd.DataFrame,      # ← required: discount product list
    ...
    # Apply month labels safely (drops Total/empty rows)
    meta["_month"]    = meta["Month"].apply(make_month_label)
    meta              = meta[meta["_month"].notna()].copy()
    ...
    months_raw     = sorted(merged["Month"].dropna().unique(), key=lambda x: parse_month_start(x) or pd.Timestamp.min)
    merged["Month_Label"] = merged["Month"].apply(make_month_label)
    months_ordered = [m for m in [make_month_label(m) for m in months_raw] if m]
...
def run_product_analysis(
    ...
    # ── Apply month labels safely — drop rows where label is None ───
    meta_month_col  = find_col(meta,    "month")
    shop_month_col  = find_col(shopify, "month")

    has_month = bool(meta_month_col and shop_month_col)
    ...
    base_cols = ["Product ID","Google Item ID","Product Title","Spend","Revenue","ROI"] if _has_gid \
           else ["Product ID","Product Title","Spend","Revenue","ROI"]

    q1 = merged[(merged["Revenue"] >= rv_cut) & (merged["Spend"] <  sp_cut)][base_cols].sort_values("Revenue", ascending=False).reset_index(drop=True)
    q2 = merged[(merged["Revenue"] >= rv_cut) & (merged["Spend"] >= sp_cut)][base_cols].sort_values("Revenue", ascending=False).reset_index(drop=True)
    q3 = merged[(merged["Revenue"] <  rv_cut) & (merged["Spend"] >= sp_cut)][base_cols].sort_values("Spend",   ascending=False).reset_index(drop=True)
    q4 = merged[(merged["Revenue"] <  rv_cut) & (merged["Spend"] <  sp_cut)][base_cols].sort_values("Revenue", ascending=False).reset_index(drop=True)

    # ── Monthly aggregations (optional, for context) ─────────────────
    merged_m = pd.DataFrame()
    if has_month and "_month" in meta.columns and "_month" in shopify.columns:
        ...
        merged_m   = merged_m.rename(columns={"_pid":"Product ID","_month":"Month","_spend":"Spend","_rev":"Revenue"})
    ...
    return {
        ...
        "total_months": merged_m["Month"].nunique() if not merged_m.empty and "Month" in merged_m.columns else 0,
    }