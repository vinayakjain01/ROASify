...
export function DiscountHero({ discountedStats, nonDiscountedStats }: Props) {
  const roiAdvantage = discountedStats.roi > 0 ? nonDiscountedStats.roi / discountedStats.roi : 0;
  ...
  return (
    <div className="bg-white rounded-[10px] border border-[#EEECE5] overflow-hidden">
      <div className="p-6">
        <div className="text-xs font-medium text-[#8B8780] uppercase tracking-wider mb-3">Which strategy wins</div>
        <div className="flex items-start justify-between gap-8">
          ...
          <div className="flex-shrink-0 pl-8 border-l border-[#EEECE5]">
            <div className="text-4xl font-bold text-[#4F46E5] tabular-nums">{roiAdvantage.toFixed(2)}x</div>
        ...
        <div className="mt-6 flex items-center justify-between p-4 bg-[#FAFAF8] rounded-lg border border-[#EEECE5]">
          <p className="text-sm text-[#57544E]">
            Reallocate <span className="font-medium text-[#1A1814]">{inr(reallocationAmount)}</span> from discounted spend to your top non-discounted performers.
          </p>
          <Button className="bg-[#4F46E5] hover:bg-[#4338CA]">
            Build reallocation plan <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
        </div>
      </div>
    </div>
